...
- `docs/known-issues.md` — defects that are documented rather than fixed. Check before "fixing" something.
...
- `test/` — `node:test` suites. Requires a live Redis.
...
- `yarn setup-redis-server` — Docker `redis:7.0` on localhost:6379. Required before tests.
- `yarn test` — sets up Redis, builds, then runs `node --test ./test/**/*.test.js`. Tests exercise the
  compiled `.js`, so always build first. **Note the trailing `|| true`: the script cannot fail.**
  Read the actual output rather than the exit code.
