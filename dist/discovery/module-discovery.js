 ...
 */
export async function discoverClasses(options) {
    given(options, "options").ensureHasValue().ensureIsObject();
    const { directoryUrl, fileSuffix, classNameSuffix, kind, isMatch } = options;
    ...
    for (const filePath of filePaths) {
        ...
        if (matches.length === 0)
            throw new ApplicationException(`${kind} discovery: '${displayPath}' matched the ${kind} file convention but exports no ${kind}.`);
        for (const match of matches) {
            const name = match.name;
            if (name.startsWith("_"))
                continue;
            if (!name.endsWith(classNameSuffix))
                throw new ApplicationException(`${kind} discovery: '${name}' in '${displayPath}' does not end with '${classNameSuffix}'.`);
            const existing = seen.get(name);
            if (existing != null) {
                if (existing.value === match)
                    continue;
                throw new ApplicationException(`${kind} discovery: '${existing.path}' and '${displayPath}' both resolve to name '${name}'; registration is keyed on class.name, so names must be unique per app.`);
            }
...
async function findFiles(directory, fileSuffix, kind) {
    const entries = await readdir(directory, {
        recursive: true,
        withFileTypes: true,
    }).catch((error) => {
        if (error.code === "ENOENT")
            throw new ApplicationException(`${kind} discovery: directory '${directory}' not found.`);
        throw error;
    });
    return entries
...
//# sourceMappingURL=module-discovery.js.map