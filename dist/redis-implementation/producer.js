...
import { ATTR_MESSAGING_SYSTEM, ATTR_MESSAGING_OPERATION_NAME, ATTR_MESSAGING_OPERATION_TYPE, ATTR_MESSAGING_DESTINATION_NAME, ATTR_MESSAGING_DESTINATION_TEMPORARY, ATTR_MESSAGING_MESSAGE_ID, ATTR_MESSAGING_MESSAGE_CONVERSATION_ID } from "@opentelemetry/semantic-conventions/incubating";
import { ATTR_DB_OPERATION_NAME, ATTR_ERROR_TYPE } from "@opentelemetry/semantic-conventions";
import { ATTR_NEDA_COMPONENT, ATTR_NEDA_DIRECTION, ATTR_NEDA_EVENT_NAME, errorTypeOf, instruments, MESSAGING_SYSTEM, timeRedisCommand } from "../metrics.js";
export class Producer {
    ...
    async _compressEvents(events) {
        const startedAt = performance.now();
        try {
            return await Make.callbackToPromise(Zlib.deflateRaw)(Buffer.from(JSON.stringify(events), "utf8"));
        }
        finally {
    ...
    _incrementPartitionWriteIndex() {
        ...
        return timeRedisCommand("INCR", "producer", () => new Promise((resolve, reject) => {
