...
export class MetricsReporter {
    ...
    _isReporting = false;
    _isDisposed = false;
    constructor(brokers, logger, consumerGroupId, consumerName, interval) {
     ...
     */
    buildRecords(now) {
        return this._brokers.flatMap(broker => [...broker.metrics.entries()]
            .orderBy(t => t[0])
    ...
    async _report(now) {
        const logCalls = this.buildRecords(now).flatMap(record => {
            ...
            if (record.lag > maxAcceptableLag)
                calls.push(this._logger.logWarning(`Event queue lag for topic ${record.topic} partition ${record.partition} `
